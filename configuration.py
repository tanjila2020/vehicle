...
blind_d = [1, 2, 4, 6, 8, 10, 12, 14, 16, 20]
...
transfer_rates = np.zeros((len(blind_d), len(times))) 
...
for i in range(0,(len(blind_d))):
    ...
    for j in range(0,(len(times))):
        ...
        transfer_rates[i,j] = round((1.8/transfer_time), 2)  # transfer rate megabit per sec
...
df9 = pd.DataFrame(transfer_rates)

print("peak servers:",df7)

df1.to_csv('config_ap.csv', index = False)
df2.to_csv('config_server.csv', index = False)
df3.to_csv('response_times.csv', index = False)
df4.to_csv('avg_ap.csv', index = False)
df5.to_csv('avg_server.csv', index = False)
df6.to_csv('peak_ap.csv', index = False)
df7.to_csv('peak_server.csv', index = False)
df8.to_csv('transfer_times.csv', index = False)
df9.to_csv('transfer_rates.csv', index = False)
